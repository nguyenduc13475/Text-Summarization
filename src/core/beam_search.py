...
class BeamSearch:
    def __init__(
        ...
        self.sequences: torch.Tensor = torch.full(
        ...
        self.scores: torch.Tensor = None  # type: ignore
        self.finishes: torch.Tensor = None  # type: ignore

    def init_from_first_topk(
        self, batch_final_distributions: torch.Tensor
    ) -> torch.Tensor:
    ...
    def advance(
        self,
        batch_final_distributions: torch.Tensor,
        trigram_penalty: float = -30.0,
        bigram_penalty: float = -10.0,
    ...
    def finalize_best_beams(self) -> torch.Tensor:
